...
import luminous.unitconverter.objects.MyTempUnitConverter.MyUnit;
...
public class UnitTempConverterActivity extends SlidingBaseFragmentActivity {
	...
	private void init() {
		converter = new MyTempUnitConverter(getActivity());
		...
		mAdapter = new ConverterTempAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderTemp(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_TEMPERATURE,
					MyTempUnitConverter.CELSIUS);
			tvUnitSelVal.setText(MyTempUnitConverter.CELSIUS);
		}
		...
		lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> adapterview, View view,
				...
				Utils.setPref(getActivity(),
						Constant.SELECTED_UNIT_TEMPERATURE, data[position]);
				mAdapter.addAll(converter.getFavUnit());
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.TEMPERATURE);
