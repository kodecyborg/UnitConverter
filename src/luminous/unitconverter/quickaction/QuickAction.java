package luminous.unitconverter.quickaction;
...
public class QuickAction extends PopupWindows implements OnDismissListener {
	...
	public void addActionItem(ActionItem action) {
		...
		if (action.isSelected()) {
			text.setTypeface(Utils.getBold(mContext));
		} else {
			text.setTypeface(Utils.getNormal(mContext));
		}
	...
	public void show(View anchor) {
		...
		boolean isLeft = (anchor.getRight() > Utils.getDeviceWidth(mContext) / 2) ? false
				: true;

		showArrow(((onTop) ? R.id.arrow_down : R.id.arrow_up), arrowPos, isLeft);
		setAnimationStyle(screenWidth, anchorRect.centerX(), onTop);

		mWindow.showAtLocation(anchor, Gravity.NO_GRAVITY, xPos + 10, yPos);
	}
	 ...
	 */
	private void showArrow(int whichArrow, int requestedX, boolean isLeft) {
		...
		// this is patiyu for arrow position
		if (isLeft) {
			param.leftMargin = param.leftMargin + 5;
		} else {
			param.leftMargin = param.leftMargin - 5;
		}
