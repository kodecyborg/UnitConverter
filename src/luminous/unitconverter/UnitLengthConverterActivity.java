...
import luminous.unitconverter.adapter.ConverterLengthAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MyLengthUnitConverter;
import luminous.unitconverter.objects.MyLengthUnitConverter.MyUnit;
...
public class UnitLengthConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterLengthAdapter mAdapter;
	...
	private void init() {
		...
		mAdapter = new ConverterLengthAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderLength(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_LENGTH,
					MyLengthUnitConverter.METRE);
			tvUnitSelVal.setText(MyLengthUnitConverter.METRE);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.LENGTH);
