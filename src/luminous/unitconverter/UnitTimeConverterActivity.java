...
import luminous.unitconverter.adapter.ConverterTimeAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MyTimeUnitConverter;
import luminous.unitconverter.objects.MyTimeUnitConverter.MyUnit;
...
public class UnitTimeConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterTimeAdapter mAdapter;

	TextView tvUnitSelVal;
	TextView tvUnitValueVal;

	MyTimeUnitConverter converter;
	...
	private void init() {
		converter = new MyTimeUnitConverter(getActivity());
		...
		mAdapter = new ConverterTimeAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderTime(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_TIME,
					MyTimeUnitConverter.SECOND);
			tvUnitSelVal.setText(MyTimeUnitConverter.SECOND);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.TIME);
