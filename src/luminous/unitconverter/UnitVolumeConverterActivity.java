...
import luminous.unitconverter.adapter.ConverterVolumeAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MyVolumeUnitConverter;
import luminous.unitconverter.objects.MyVolumeUnitConverter.MyUnit;
...
public class UnitVolumeConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterVolumeAdapter mAdapter;

	TextView tvUnitSelVal;
	TextView tvUnitValueVal;

	MyVolumeUnitConverter converter;
	...
	private void init() {
		converter = new MyVolumeUnitConverter(getActivity());
		...
		mAdapter = new ConverterVolumeAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderVol(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_VOLUME,
					MyVolumeUnitConverter.CUBIC_METRE);
			tvUnitSelVal.setText(MyVolumeUnitConverter.CUBIC_METRE);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.VOLUME);
