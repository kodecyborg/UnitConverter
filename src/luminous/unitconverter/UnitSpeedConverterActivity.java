...
import luminous.unitconverter.adapter.ConverterSpeedAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MySpeedUnitConverter;
import luminous.unitconverter.objects.MySpeedUnitConverter.MyUnit;
...
public class UnitSpeedConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterSpeedAdapter mAdapter;

	TextView tvUnitSelVal;
	TextView tvUnitValueVal;

	MySpeedUnitConverter converter;
	...
	private void init() {
		converter = new MySpeedUnitConverter(getActivity());
		...
		mAdapter = new ConverterSpeedAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderSpeed(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_SPEED,
					MySpeedUnitConverter.METRE_PER_SEC);
			tvUnitSelVal.setText(MySpeedUnitConverter.METRE_PER_SEC);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.SPEED);
