...
import luminous.unitconverter.adapter.ConverterAreaAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MyAreaUnitConverter;
import luminous.unitconverter.objects.MyAreaUnitConverter.MyUnit;
...
public class UnitAreaConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterAreaAdapter mAdapter;
	...
	private void init() {
		converter = new MyAreaUnitConverter(getActivity());
		...
		mAdapter = new ConverterAreaAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderArea(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_AREA,
					MyAreaUnitConverter.SQUARE_METRE);
			tvUnitSelVal.setText(MyAreaUnitConverter.SQUARE_METRE);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.AREA);
