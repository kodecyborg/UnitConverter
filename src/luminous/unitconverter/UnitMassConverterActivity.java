...
import luminous.unitconverter.adapter.ConverterMassAdapter;
import luminous.unitconverter.dslv.DragSortListView;
import luminous.unitconverter.objects.MyMassUnitConverter;
import luminous.unitconverter.objects.MyMassUnitConverter.MyUnit;
...
public class UnitMassConverterActivity extends SlidingBaseFragmentActivity {

	DragSortListView lvUnitConverter;
	ConverterMassAdapter mAdapter;
	...
	private void init() {
		...
		mAdapter = new ConverterMassAdapter(getActivity(),
		...
		public void drop(int from, int to) {
			if (from != to) {
				try {
					...
					Utils.saveUnitOrderMass(getActivity(), mAdapter.getAll());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void checkUnitAndRefresh() {
		...
		if (!unitList.contains(selectedUnit)) {
			Utils.setPref(getActivity(), Constant.SELECTED_UNIT_MASS,
					MyMassUnitConverter.KILOGRAM);
			tvUnitSelVal.setText(MyMassUnitConverter.KILOGRAM);
		}
				...
				.setOnActionItemClickListener(new QuickAction.OnActionItemClickListener() {
					...
					public void onItemClick(QuickAction source, int pos,
						...
						if (actionItem.getActionId() == ID_ADD_UNIT) {
							...
							intent.putExtra("unit_type", MenuItem.MASS);
